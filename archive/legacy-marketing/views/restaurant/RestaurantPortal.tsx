...
import { 
  Lock, Mail, LogOut, Utensils, ChefHat, LayoutDashboard, AlertCircle 
} from "lucide-react";
...
export default function RestaurantPortal() {
      ...
      <div className="min-h-screen bg-[#09090b] bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(120,119,198,0.15),rgba(255,255,255,0))] flex items-center justify-center p-4 font-sans">
        <div className="w-full max-w-md bg-white/5 border border-white/10 rounded-[32px] p-8 shadow-2xl backdrop-blur-md relative overflow-hidden">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full h-11 bg-violet-600 hover:bg-violet-750 text-white rounded-xl font-bold text-xs cursor-pointer shadow-lg shadow-violet-600/20 transition-all border-0 mt-2 hover:-translate-y-0.5 active:translate-y-0"
            >
  ...
  return (
    <div className="min-h-screen bg-[#070708] text-slate-100 flex font-sans">
      {/* Dynamic Isolated Left Sidebar */}
      <aside className="w-64 border-r border-white/5 bg-[#09090b]/80 backdrop-blur-md flex flex-col justify-between shrink-0 select-none">
        <div>
          {/* Header Branding */}
          <div className="p-6 border-b border-white/5 flex items-center gap-3">
            <div className="h-9 w-9 rounded-xl bg-violet-600/10 border border-violet-500/20 flex items-center justify-center text-violet-400">
              <ChefHat size={18} />
            </div>
            <div>
        ...
        <div className="p-4 border-t border-white/5 bg-white/[0.01]">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 h-9 border border-white/10 hover:border-rose-900 bg-transparent hover:bg-rose-950/20 text-slate-400 hover:text-rose-400 rounded-xl font-bold text-[10px] uppercase cursor-pointer transition-all active:scale-98"
          >
            <LogOut size={12} />
            Exit Operations
          </button>
      ...
      <main className="flex-1 overflow-y-auto p-8 relative">
        ...
        <div className="flex justify-between items-center mb-8 border-b border-white/5 pb-4">
          <div>
            <h1 className="text-xl font-black text-white">Vertical POS Terminal</h1>
            <p className="text-xs text-slate-500 font-medium">Manage tables, orders, kitchen queue, staff and menus.</p>
          </div>
