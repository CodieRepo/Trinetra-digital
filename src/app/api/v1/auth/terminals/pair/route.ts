 ...
 * Route: POST /api/v1/auth/terminals/pair
 * Description: Pure transport handler for hardware terminal device pairing.
 */
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await terminalPairingService.pairTerminal(body);
    return createSuccessResponse(result, traceId, 201);
  } catch (error) {
