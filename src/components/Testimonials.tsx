...
export default function Testimonials() {
  return (
    <section id="portfolio" className="relative overflow-hidden bg-white py-16 md:py-24">
      <div className="max-w-[1200px] mx-auto px-4 md:px-10 relative z-10">

        {/* Stats Row */}
        <div className="grid gap-0 md:grid-cols-3 mb-20 border border-border rounded-2xl overflow-hidden">
          {CAPABILITIES.map((cap, i) => (
            <motion.div
              ...
              className="flex flex-col items-center justify-center p-8 text-center border-b md:border-b-0 md:border-r border-border last:border-0"
            >
              <span className="font-display text-[52px] leading-none text-slate-900 font-semibold mb-2 tracking-tight">
              ...
              return (
                ...
                >
                  ...
                  <div className={`${isEven ? "lg:order-1" : "lg:order-2"} group`}>
                    ...
                    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-md transition-shadow duration-300 group-hover:shadow-lg">
                      {/* Browser bar */}
                      <div className="bg-slate-50 border-b border-slate-200/80 px-4 py-2.5 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1.5 bg-white border border-slate-200/60 rounded-md px-4 py-1 text-[9px] font-mono text-slate-400 font-semibold tracking-wide">
                          <span className="h-1.5 w-1.5 rounded-full bg-green-400" />
                          {item.url}
                        </div>
                        <ExternalLink size={10} className="text-slate-400" />
                      </div>
                      {/* Video area — correct aspect ratio with proper height */}
                  ...
                  <div className={`${isEven ? "lg:order-2" : "lg:order-1"} space-y-6 text-left`}>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-accent-light text-sm font-mono font-bold text-accent border border-accent/15">
                        {item.avatar}
                      </div>
                      <div>
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          ...
                          <span className="text-[8px] font-mono font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-[#E2F2E9] text-[#137A3E] border border-[#A3E0BA] ml-1">
                            {item.status}
                          </span>
                        </div>
                      </div>
                    ...
                    <div>
                      <p className="text-[9px] font-mono font-bold uppercase tracking-widest text-slate-400 mb-3">
                        Services Delivered
                      </p>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="mt-20 text-center p-6 md:p-8 rounded-2xl border border-border bg-[#F8FAFC]"
        >
          <p className="text-xs font-bold text-slate-900 font-mono uppercase tracking-wider mb-2">
